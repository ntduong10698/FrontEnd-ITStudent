...
function viewField(text) {
   //text mang giá trị: 1, string, boolean, đối tượng, ..: true
   //text là null hoặc undefiend, ..: false.
    return text ? text : "";
}

function clickAddCart(productId) {
    alertSuccess(`Thêm thành công product: ${productId}`);
}

function viewSrcFile(src) {
    return src ? src.indexOf("http") == 0 ? src : URL_FILE  + src : "";
}
